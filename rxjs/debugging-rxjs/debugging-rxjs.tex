...
\mybox{In practice, something like this should ideally be tested using unit tests. However, many enterprise applications unfortunately are not built in this optimal fashion for various reasons.}
...
\mybox{I've seen one other article in particular claim use of a debug utility functionality that is only turned on in development mode. I personally am against such debugging patterns, and feel it is an anti-pattern. I strongly believe that unit tests should be in place to prevent errors. Debugging should happen in one off scenarios, and then reinforced with unit tests and integration tests to make sure error does not happen again.}