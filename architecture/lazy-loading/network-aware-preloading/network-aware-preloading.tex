...
above), and the amount of time saved, to be honest, is negligible. However, I also
...
tie in custom events into already existing custom pre-loading. In particular, we 
...
\lstinline{CustomPreloadingService} to trigger preloading: 

\begin{lstlisting}[caption=on-demand-preloading.service.ts]
...
Now we will be integrating our \lstinline{OnDemandPreloadService} 
with our \lstinline{CustomPreloadingService}
...
take around an entire second. In such a slow network, if the user hovers
...
will not support it as part of it's core library yet. So we will simply declare a 
var for navigator to make it go away. Let's add a has \lstinline{GoodConnection} 
function to our \lstinline{custom-preloading.ts} file, so that we can tell 
whether, or not a user has a good connection. 
...
You will notice, that we have added an additional condition for preload function. 
